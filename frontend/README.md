# Frontend — 틈나는 시간 여행 추천 웹앱

React + TypeScript + Vite 기반 프론트엔드. 위치·틈나는 시간·이동수단을 입력받아 백엔드 추천 API를 호출하고, 결과를 카카오 지도와 목록으로 보여준다. 한국어/영어/일본어 다국어를 지원하며(기본 한국어), 상단 우측에서 언어를 전환할 수 있다.

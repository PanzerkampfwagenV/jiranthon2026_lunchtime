...
- [ ] 저장소 구조 결정 (monorepo vs FE/BE 분리)
- [ ] 사용할 지도/길찾기 API 확정 (Kakao Map, Naver, Google 중 택1)
- [ ] **API 인터페이스 계약(Contract) 정의** — 요청/응답 JSON 스키마 합의 (아래 참고)
- [ ] 브랜치 전략 / 커밋 컨벤션 / 코드 리뷰 규칙 합의
- [ ] 환경변수(.env) 및 API 키 관리 방식 합의 (키는 저장소에 커밋 금지)
- [ ] 디자인 시안 / 와이어프레임 공유
...
- [ ] 라우팅 구성 (첫 화면 → 결과 화면)
- [ ] 공통 UI 컴포넌트 / 스타일 시스템 세팅 (예: Tailwind, CSS Modules)

### 첫 화면 & 위치 입력
- [ ] 첫 화면(랜딩) 레이아웃 구현
- [ ] **현재 위치(GPS)** 가져오기 — `navigator.geolocation` 연동
- [ ] GPS 권한 거부 / 실패 시 예외 처리 및 안내 UI
- [ ] **지정 위치 입력** — 주소/장소 검색 자동완성 (지도 API 연동)
- [ ] 위치 선택 결과를 지도 마커/좌표로 확정

### 자투리 시간 입력
- [ ] 자투리 시간 입력 UI (슬라이더 / 프리셋 버튼 30·60·90분 / 직접 입력)
- [ ] 이동 수단 선택 옵션 (도보 / 대중교통 / 자동차)
- [ ] 입력값 유효성 검증 (시간 범위, 위치 필수 등)

### 상태 관리 & 연동
- [ ] 전역 상태 관리 세팅 (Context/Zustand 등)
- [ ] 추천 요청 API 호출 및 로딩/에러 상태 처리
- [ ] 결과 화면으로 데이터 전달
...
### 추천 결과 화면
- [ ] 추천 결과 목록 화면 레이아웃
- [ ] 장소 카드 컴포넌트 (이름, 카테고리, 이동시간, 거리, 썸네일)

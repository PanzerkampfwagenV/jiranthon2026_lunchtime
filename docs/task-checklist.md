...
### 첫 화면 & 위치 입력 (`M-01`)
- [x] 첫 화면(랜딩) 레이아웃 구현
- [x] 첫 화면 감성 UI — 서울 사진 폴라로이드 배경 콜라주 + iOS 글래스모피즘 (`6c2c7ee`, `8080882`)
- [x] 테마 선택(시스템/라이트/다크) + 유리효과 설정 (`8080882`)
- [x] **현재 위치(GPS)** 가져오기 — `navigator.geolocation` 연동
- [x] GPS 권한 거부 / 실패 시 예외 처리 및 안내 UI
- [x] **지정 위치 입력** — 주소/장소 검색 자동완성 (카카오 장소 검색 API 연동) (`5b1fbdd`)
- [x] 위치 선택 결과를 지도 마커/좌표로 확정

### 틈나는 시간 입력 (`M-02`, `M-03`, `M-04`)
- [x] 틈나는 시간 입력 UI (슬라이더 10~180분 / 프리셋 버튼 30·60·90·120분 / 직접 입력)
- [x] 이동 수단 선택 옵션 (도보 / 대중교통 / 자동차)
- [x] 입력값 유효성 검증 (시간 범위, 위치 필수 등)
- [x] **해시태그 기반 설정 UI** — 위치/자투리시간/이동수단을 #해시태그로 표현, 클릭 시 상세 설정 펼침 (`c1b84b0`)

### 선택 옵션 (개인화)
- [x] **#MBTI** — 16유형 선택 카드 패널, `SearchContext.mbti` 상태 (`a63db2d`) *(추천 로직 반영은 추후)*
- [x] **#럭키데이** — 생년월일/시각/양력·음력/성별 입력, `SearchContext.luckyDay` 상태 (`ade0afb`) *(운세 기반 추천 로직은 추후)*
...
- [x] 경로/이동시간 시각화 (선택) — 자동차: OSRM 실제 도로 경로+시간, 도보/대중교통: 직선 대체 (`5108486`, `c4446fb`)
- [x] 장소 이름표 CustomOverlay 표시 — InfoWindow 이중 창 버그 수정 (`660f153`)

### 상세 & 마무리 (`E-06`)
- [x] 장소 상세 화면/모달 (설명, 사진, 외부 지도 길찾기 링크) — `PlaceDetailModal`
- [x] 상세 모달 LLM 기반 활동/하이라이트/소개 표시 — `GET /api/place-detail` 연동 (`38fc27f`)
...
### 외부 API 연동 (`M-06`)
- [x] 지도/장소 검색 API 연동 (주변 여행지 조회) — 카카오 로컬 검색으로 LLM 생성 장소명 좌표 보정 (`3fdf421`)
- [x] 길찾기/이동시간 API 연동 (출발지 → 각 장소 소요시간) — OSRM 실제 도로 거리·시간 보정, `GET /api/route` 경로 조회 (`eca3534`, `3e3a8c8`) *(대중교통은 직선 대체 유지)*
- [x] LLM 기반 장소 상세 정보 API — `GET /api/place-detail` 활동/하이라이트/소개, 1시간 TTL 캐시 (`5984262`)
- [ ] 외부 API 응답 캐싱 / 호출 최적화 *(place-detail 캐시 도입, 추천/경로 캐싱은 미도입)*
...
- [x] **LLM 기반 장소 추천** — 사내 게이트웨이(Claude)로 조건 맞춤 장소명 생성, 실패 시 규칙 기반 폴백 (`3fdf421`)
- [x] 입력 위치 기준 후보 장소 수집 — `data/places.ts` 시드 12곳
- [x] **틈나는 시간 내 도달 가능 여부 필터링** — 편도/왕복(`tripType`) 기준 적용
...
- [x] 주요 시나리오 E2E 확인 (GPS 추천 / 지정 위치 추천 / 시간대별) *(Playwright E2E 도입, 지정 위치 추천 흐름 자동 검증 `98e02bb`)*

...
- 원인은 OSRM 기반 거리 재보정(다른 개발자의 이전 작업)이 도보 모드에서 haversine 직선거리보다 훨씬 긴 우회 거리를 산출해, 자투리 시간 필터에서 정상 후보까지 모두 걸러버린 것이었습니다.
...
60분 자투리 시간(왕복 기준 편도 30분 = 약 1.8km 이내)처럼 짧은 구간일수록 이 배율 차이의 영향이 커서, 도보 모드는 거의 모든 조건에서 결과가 0곳이 되는 회귀가 발생했다.
